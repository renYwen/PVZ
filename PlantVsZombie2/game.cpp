...
void Init()//初始化
{
	thread Thread(&Resources::Init, &ResourcePool);//创建资源加载线程
...
void Update()//对象及组件更新或删除
{
	if (!pause) {
		...
		for (auto it = GameColliders.begin(); it != GameColliders.end(); ++it)
		{
			...
			for (int i = x; i <= x_1; ++i)
				for (int j = y; j <= y_1; ++j)
					ColliderZones[i][j].push_back(*it);
		}
		collisionCount = 0;
		for (auto&arr_i: ColliderZones){
			for (auto& arr_j : arr_i){
		...
		if (!GameObjects_.empty()) {
			for (auto it_ = GameObjects_.begin(); it_ != GameObjects_.end(); ++it_)
			{
		...
		if (!GameComponents_.empty()) {
			for (auto it1_ = GameComponents_.begin(); it1_ != GameComponents_.end(); ++it1_)
			{
