...
AChomper::AChomper()
{
	...
	ChompTimeHandle.Bind(40.f,&AChomper::EatUp,this,true);
}
...
void AChomper::Update()
{
	...
	if (!state)
	{
		...
		if (!buf.empty()) {
			...
			player->Play("flower_eat");
		}
	}
	else if (state == 1)
	{
		if (ani->GetNode()->GetAni().index == 0 && eatFlag) {
			state = 2; ani->SetNode("chomp"); ChompTimeHandle.ReSet();
			eatFlag = false;
		}
			...
			if (!buf.empty()) {
				for (auto it = buf.begin(); it != buf.end(); it++) {
					if (AZombie* pZombie = Cast<AZombie>(*it)) {
						if (pZombie->GetName() != AZombie::BossZombie) { pZombie->Destroy(); zombieNumPerRow[row]--; break; }
