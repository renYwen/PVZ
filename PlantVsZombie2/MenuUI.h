...
public:
	UMenuUI()
	{
		...
		Leaf[0] = AddWidget<Image>(Vector(130, 525));
		Leaf[0]->SetSize(Vector(40, 40));
		Leaf[1] = AddWidget<Image>(Vector(150, 530));
		Leaf[1]->SetSize(Vector(34, 33));
		Leaf[2] = AddWidget<Image>(Vector(170,505));
		Leaf[2]->SetSize(Vector(49, 66));
		Leaf[3] = AddWidget<Image>(Vector(215, 525));
		Leaf[3]->SetSize(Vector(32, 45));
		Leaf[4] = AddWidget<Image>(Vector(250, 530));
		Leaf[4]->SetSize(Vector(25, 37));
		for (int i = 0; i < 5;i++) {
			Leaf[i]->SetAttachment(NearBg);
			sprintf_s(cardname, sizeof(cardname), "res/images/Menu/SelectorScreen_Leaf%d.png", i + 1);
			Leaf[i]->LoadPicture(cardname);
			Leaf[i]->SetLayer(6+i);
		}
